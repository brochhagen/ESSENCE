...
def summarize_counts(lst,states,messages): #Auxiliary function to summarize state-message production observations
...
def get_obs(s_amount,m_amount,k,likelihoods,state_freq,sample_amount): #Get observations as samples from production behavior of a linguistic type
...
def get_likelihood(obs,likelihoods): #Get likelihood of types given a set of observations
...
def get_mutation_matrix(s_amount,m_amount,state_freq, likelihoods,lexica_prior,learning_parameter,sample_amount,k,lam,alpha,mutual_exclusivity): #Get mutation/learning matrix Q
...
def get_utils(typeList,states,messages,lam,alpha,mutual_exclusivity): #Get utility matrix U
...
def get_type_bin(type_idx,bins): #Bin types according to lexica permutations of concepts
    for b in xrange(len(bins)):
        if type_idx in bins[b]:
            return b
    

def run_dynamics(alpha,lam,k,sample_amount,gens,runs,states,messages,learning_parameter,kind,mutual_exclusivity): #Main function to run dynamics
